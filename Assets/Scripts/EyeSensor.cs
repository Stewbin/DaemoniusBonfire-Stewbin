...
public class EyeSensor : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for (int i = 1; i < rays.Length; i++)
        {
            ...
            RaycastHit2D hit = Physics2D.Raycast(transform.position, new Vector2(Mathf.Cos(angleRad), Mathf.Sin(angleRad)), 5);
            rays[i] = v;
    ...
    public bool isNear()
    {
        ...
        {
            RaycastHit2D hit = Physics2D.Raycast(transform.position, rays[i], 5);
            if (hit.collider != null)
            {
                return true;
            }
